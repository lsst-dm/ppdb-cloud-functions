...
# Set the time-to-live for staging tables in BigQuery so that they expire
# automatically after two hours.
_STAGING_TABLE_TTL = datetime.timedelta(hours=2)

# Type for table name, fully qualified staging table, and fully qualified
# internal table
...
def read_schema_columns(parquet_path: str) -> list[str]:
    """Return data columns from parquet file, excluding masks and pandas index
    artifacts.
...
def sanitize_row(row: dict[str, Any]) -> dict[str, Any]:
    ...
    ----------
    row
        The dictionary representing a single row of data.
    """
    return {
...
def write_to_bigquery(
    ...
    ----------
    pcoll
        The PCollection containing the rows to be written to BigQuery.
    table_name
        The name of the table corresponding to the PCollection.
    table_fqn
        The fully qualified name of the BigQuery table.
    temp_location
        The GCS location for temporary files used during the load operation.
    """
    log_event(
        logging.INFO,
...
def create_staging_table(
    ...
    ----------
    bq_client
        The client used to interact with the BigQuery service.
    staging_ref
        The fully qualified reference to the staging table.
    internal_ref
        The fully qualified reference to the internal table.
    table_name
        The name of the table corresponding to the staging and internal tables.
    """
    log_event(
        logging.INFO,
...
class CreateStagingTable(apache_beam.DoFn):
    ...
    ----------
    project_id
        The ID of the GCP project containing the BigQuery datasets.
    """
    ...
    def process(self, table_spec: TableSpec) -> Iterator[str]:
        ...
        ----------
        table_spec
            The specification of the table to create, including its name and
            references to the staging and internal tables.
        """
...
class PromoteTables(apache_beam.DoFn):
    ...
    project_id
        The ID of the GCP project containing the BigQuery datasets.
    table_specs
        A list of table specifications, each including the table name and
        references to the staging and internal tables.
    """
    ...
    def process(self, unused_element: None) -> Iterator[str]:
        ...
        ----------
        unused_element
            A placeholder element to trigger the promotion after all staging
...
def run(argv: list[str] | None = None) -> None:
    ...
    ----------
    argv
        A list of command-line arguments to pass to the pipeline. If None,
        ...
        with apache_beam.Pipeline(options=options) as pipeline:
            ...
            for table_name, staging_ref, _ in table_specs:
                ...
                data_after_table_creation = data | (
                    f"WaitFor{table_name}StagingTable" >> WaitOn(staging_tables_ready)
                )
